...
async function resolveConfigValue(key) {
  try {
    const config = await db.select().from(systemConfig).where(eq(systemConfig.chave, key)).limit(1);
...
async function analyzeImageWithOpenRouter(imageUrl, prompt) {
