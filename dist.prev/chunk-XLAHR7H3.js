import {
  ...
  getAdminMediaByName,
  parseAdminMediaTags
} from "./chunk-F27SWI4I.js";
import {
  insertAgentMedia
} from "./chunk-457TZRNW.js";
import {
  cancelFollowUp,
  followUpService,
...
function getGuidedBusinessQuestion() {
  return "Perfeito. Pra eu criar seu agente do jeito certo, me responde 3 coisas rapidinho. 1) Qual \xE9 o nome do seu neg\xF3cio e qual \xE9 o principal servi\xE7o ou produto que voc\xEA vende?";
}
...
async function applyStructuredSetupToUser(userId, session) {
  ...
  if (workflowKind === "scheduling" && profile?.usesScheduling !== false) {
    await supabase.from("scheduling_config").upsert(
      {
        user_id: userId,
        is_enabled: true,
...
function buildFastAdminFallback(session, userMessage) {
  ...
  const profile = session.setupProfile;
  let pendingGuidedQuestion = getGuidedBusinessQuestion();
...
async function maybeHandleGuidedOnboardingTurn(session, userMessage) {
  if (session.userId || session.flowState !== "onboarding") {
  ...
  if (!profile.answeredBusiness) {
    if (!cleanMessage || isSimpleGreetingMessage(userMessage)) {
      ...
      return {
        handled: true,
        text: getGuidedBusinessQuestion(),
        shouldCreate: false
      };
    }
...
[SISTEMA: O usu\xC3\xA1rio enviou uma m\xC3\xADdia do tipo ${mediaType}. Se for imagem/\xC3\xA1udio sem contexto, pergunte o que \xC3\xA9 (ex: cat\xC3\xA1logo, foto de produto, etc).]`;
  ...
  const { getSmartTriggers } = await import("./adminMediaStore-5VMLISCL.js");
