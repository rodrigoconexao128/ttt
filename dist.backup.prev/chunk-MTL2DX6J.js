...
Responda de forma concisa (m\xE1ximo 3 frases) descrevendo o que voc\xEA v\xEA.`;
  ...
  async resetClientByPhone(phoneNumber) {
    ...
    console.log(`\u{1F5D1}\uFE0F [RESET CLIENT] Iniciando reset para ${phoneNumber}...`);
    try {
   ...
   */
  async resetTestAccountSafely(phoneNumber) {
    try {
      console.log(`\u{1F50D} [SAFE RESET] Verificando seguran\xE7a para ${phoneNumber}...`);
      const [user] = await db.select().from(users).where(eq(users.phone, phoneNumber));
      if (!user) {
        console.log(`\u26A0\uFE0F [SAFE RESET] Nenhum usu\xE1rio encontrado para ${phoneNumber}`);
        const adminConv = await this.getAdminConversationByPhone(phoneNumber);
        if (adminConv) {
      ...
      console.log(`\u2705 [SAFE RESET] Valida\xE7\xF5es OK para ${phoneNumber}. Procedendo com reset...`);
  ...
  async getQuickReplies(adminId) {
    const { adminQuickReplies } = await import("./schema-Y7T5SVKW.js");
  ...
  async getUserQuickReplies(userId) {
    const { userQuickReplies } = await import("./schema-Y7T5SVKW.js");
  ...
  async isNumberExcluded(userId, phoneNumber) {
    const { exclusionList, exclusionConfig } = await import("./schema-Y7T5SVKW.js");
  ...
  async isNumberExcludedFromFollowup(userId, phoneNumber) {
    const { exclusionList } = await import("./schema-Y7T5SVKW.js");
    const { or: or2 } = await import("drizzle-orm");
    const config = await this.getExclusionConfig(userId);
    if (config && config.isEnabled === false) {
      console.log(`\u{1F6AB} [EXCLUSION] Lista de exclus\xE3o DESATIVADA explicitamente para usu\xE1rio ${userId}`);
  ...
  async getExclusionConfig(userId) {
    const { exclusionConfig } = await import("./schema-Y7T5SVKW.js");
  ...
  async getDailyUsage(userId) {
    const { dailyUsage } = await import("./schema-Y7T5SVKW.js");
    const today = /* @__PURE__ */ new Date();
    today.setHours(0, 0, 0, 0);
