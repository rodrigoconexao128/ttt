import {
  ...
  getAdminMediaByName,
  parseAdminMediaTags
} from "./chunk-NSURUCZ3.js";
import {
  insertAgentMedia
} from "./chunk-2WNPL7BX.js";
import {
  storage
} from "./chunk-CS2SLRJ6.js";
import {
  cancelFollowUp,
  followUpService,
...
function inferBusinessNameFromReply(userMessage, session) {
  if (!assistantAskedForBusinessName(session)) return void 0;
  if (looksLikeQuestionMessage(userMessage)) return void 0;
  ...
  if (blockedReplies.has(normalized)) return void 0;
  return sanitizeCompanyName(userMessage);
}
...
async function createTestAccountWithCredentials(session) {
  try {
    ...
    const { supabase } = await import("./supabaseAuth-X3PXZTUT.js");
    const applyAgentConfig = async (targetUserId) => {
      ...
      const hasIncomingConfig = Boolean(
      ...
      if (!hasIncomingConfig && existingConfig?.prompt && existingIdentity.company) {
        return {
...
S\xC3\u201C USE [ACAO:CRIAR_CONTA_TESTE] QUANDO:
- o cliente pedir para montar o teste agora
- OU a \xC3\u0161LTIMA mensagem dele for claramente o nome do neg\xC3\xB3cio que voc\xC3\xAA acabou de pedir
...
function buildFastAdminFallback(session, userMessage) {
  ...
  const firstName = /^cliente$/i.test(firstNameCandidate) ? "" : firstNameCandidate;
  ...
  if (asksIfWorthIt) {
    return `${greetingPrefix} Vale a pena quando voce quer parar de perder tempo respondendo tudo manualmente e quer mais constancia no atendimento. O AgenteZap deixa um funcionario digital atendendo, explicando seu servico e ajudando a vender no WhatsApp mesmo quando voce nao consegue responder na hora. Se quiser, eu monto um teste gratuito pra voce ver isso funcionando na pratica. Qual e o nome do seu negocio?`;
  }
  if (asksForMoreDetails) {
    return `${greetingPrefix} O AgenteZap coloca um funcionario digital no seu WhatsApp para atender, responder duvidas, apresentar seu servico e ajudar a vender como se fosse da sua equipe. Eu configuro tudo com as informacoes do seu negocio, deixo o teste pronto e depois voce pode conectar o seu numero para ele atender de verdade. Se quiser, eu ja monto seu teste gratuito por aqui e te entrego o link pronto. Qual e o nome do seu negocio?`;
  }
  if (asksHowItWorks) {
    return `${greetingPrefix} Funciona no seu proprio WhatsApp: eu configuro seu agente, depois voce conecta o seu numero no painel e ele passa a responder no seu atendimento como se fosse um funcionario seu. Se voce quiser, eu ja monto o teste gratuito por aqui e te entrego pronto para conhecer. Qual e o nome do seu negocio?`;
  }
  if (hasGreeting) {
    return `${greetingPrefix} Tudo certo por aqui. Aqui e o Rodrigo, da AgenteZap. Se voce quiser, eu posso montar um teste gratuito do seu agente por aqui, deixar pronto e te mandar o link para conhecer funcionando. Me fala o nome do seu negocio que eu comeco.`;
  }
...
[SISTEMA: O usu\xC3\xA1rio enviou uma m\xC3\xADdia do tipo ${mediaType}. Se for imagem/\xC3\xA1udio sem contexto, pergunte o que \xC3\xA9 (ex: cat\xC3\xA1logo, foto de produto, etc).]`;
  ...
  const { getSmartTriggers } = await import("./adminMediaStore-NWVPDGJO.js");
