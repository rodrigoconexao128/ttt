import {
  ...
  getAdminMediaByName,
  parseAdminMediaTags
} from "./chunk-OD3AYRWE.js";
import {
  insertAgentMedia
} from "./chunk-2OIQAKTO.js";
import {
  storage
...
async function generateTestToken(userId, agentName, company) {
  ...
  try {
    const { supabase } = await import("./supabaseAuth-NTOM73WR.js");
...
async function processAdminMessage(phoneNumber, messageText, mediaType, mediaUrl, skipTriggerCheck = false) {
  ...
  if (deleteMatch) {
    ...
    let targetMediaDesc;
    if (session.userId) {
      const { agentMediaLibrary } = await import("./schema-Y7T5SVKW.js");
      const { eq, and } = await import("drizzle-orm");
      const { db } = await import("./db-GDXAG4XZ.js");
...
[SISTEMA: O usu\xE1rio enviou uma m\xEDdia do tipo ${mediaType}. Se for imagem/\xE1udio sem contexto, pergunte o que \xE9 (ex: cat\xE1logo, foto de produto, etc).]`;
  ...
  const { getSmartTriggers } = await import("./adminMediaStore-D7NHCRPR.js");
...
async function generateFollowUpResponse(phoneNumber, context) {
  ...
  try {
    ...
    if (session && session.conversationHistory.length > 0) {
      ...
      try {
        const { adminMessages } = await import("./schema-Y7T5SVKW.js");
