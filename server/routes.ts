...
LEMBRE-SE: Você é o Rodrigo, atendente humano. Seja natural e prestativo!`;
  ...
  // ==================== TEMP CLIENTS & SCHEDULING ROUTES ====================
  
  // GET - Lista todos os clientes temporários
  app.get("/api/admin/temp-clients", isAdmin, async (_req, res) => {
    try {
      const { tempClientService } = await import("./tempClientService");
      const { db } = await import("./db");
      const { tempClients } = await import("@shared/schema");
      
      const clients = await db.select().from(tempClients).orderBy(tempClients.createdAt);
      const stats = await tempClientService.getStats();
      
      res.json({ clients, stats });
    } catch (error) {
      console.error("Error fetching temp clients:", error);
      res.status(500).json({ message: "Failed to fetch temp clients" });
    }
  });

  // GET - Detalhes de um cliente temporário
  app.get("/api/admin/temp-clients/:id", isAdmin, async (req, res) => {
    try {
      const { tempClientService } = await import("./tempClientService");
      const client = await tempClientService.getById(req.params.id);
      ...
      console.error("Error fetching temp client:", error);
      res.status(500).json({ message: "Failed to fetch temp client" });
    }
  });

  // DELETE - Remove um cliente temporário
  app.delete("/api/admin/temp-clients/:id", isAdmin, async (req, res) => {
    try {
      const { db } = await import("./db");
      const { tempClients } = await import("@shared/schema");
      const { eq } = await import("drizzle-orm");
      
      await db.delete(tempClients).where(eq(tempClients.id, req.params.id));
      
      res.json({ success: true });
    } catch (error) {
      console.error("Error deleting temp client:", error);
      res.status(500).json({ message: "Failed to delete temp client" });
    }
  });

  // POST - Marca pagamento como recebido
  app.post("/api/admin/temp-clients/:id/payment-received", isAdmin, async (req, res) => {
    try {
      const { tempClientService } = await import("./tempClientService");
      const client = await tempClientService.registerPayment(req.params.id);
  ...
  // POST - Converte cliente temporário para conta real
  app.post("/api/admin/temp-clients/:id/convert", isAdmin, async (req, res) => {
    try {
      const { tempClientService } = await import("./tempClientService");
      const { email, password } = req.body;
      ...
      const result = await tempClientService.convertToRealUser(
        req.params.id,
        email,
        hashedPassword
      );
      ...
      console.error("Error converting temp client:", error);
      res.status(500).json({ message: "Failed to convert client" });
    }
  });

  // ==================== SCHEDULED FOLLOW-UPS ROUTES ====================

  // GET - Lista todos os agendamentos de follow-up
  app.get("/api/admin/scheduled-follow-ups", isAdmin, async (req, res) => {
    try {
      const { tempClientService } = await import("./tempClientService");
      const status = req.query.status as string | undefined;
      
      const followUps = await tempClientService.getAllScheduledFollowUps(status);
  ...
  app.post("/api/admin/scheduled-follow-ups", isAdmin, async (req, res) => {
    try {
      const { followUpEngine } = await import("./followUpEngine");
      const { phoneNumber, message, scheduledFor, tempClientId } = req.body;
      
      if (!phoneNumber || !message || !scheduledFor) {
        return res.status(400).json({ message: "phoneNumber, message and scheduledFor required" });
      }
      
      await followUpEngine.scheduleManualFollowUp(
        phoneNumber,
        message,
  ...
  app.delete("/api/admin/scheduled-follow-ups/:id", isAdmin, async (req, res) => {
    try {
      const { db } = await import("./db");
      ...
      await db
        ...
        .where(eq(scheduledFollowUps.id, req.params.id));
  ...
  // GET - Estatísticas do follow-up engine
  app.get("/api/admin/follow-up-stats", isAdmin, async (_req, res) => {
    try {
      const { followUpEngine } = await import("./followUpEngine");
      const stats = await followUpEngine.getStats();
  ...
  // POST - Inicia/para o follow-up engine
  app.post("/api/admin/follow-up-engine/:action", isAdmin, async (req, res) => {
    try {
      const { followUpEngine } = await import("./followUpEngine");
      ...
      if (action === "start") {
        ...
        res.json({ success: true, message: "Follow-up engine started" });
      } else if (action === "stop") {
        followUpEngine.stop();
        res.json({ success: true, message: "Follow-up engine stopped" });
      } else {
        res.status(400).json({ message: "Invalid action. Use 'start' or 'stop'" });
      }
    } catch (error) {
      console.error("Error controlling follow-up engine:", error);
      res.status(500).json({ message: "Failed to control engine" });
    }
  });

  // GET - Status do novo fluxo de vendas
  app.get("/api/admin/sales-flow/status", isAdmin, async (_req, res) => {
    try {
      const enabled = await storage.getSystemConfig("sales_flow_enabled");
      const { tempClientService } = await import("./tempClientService");
      const { followUpEngine } = await import("./followUpEngine");
      
      const stats = await tempClientService.getStats();
      const engineStats = await followUpEngine.getStats();
      
      res.json({
        enabled: enabled?.valor === "true",
        tempClients: stats,
        followUpEngine: engineStats,
      });
    } catch (error) {
      console.error("Error getting sales flow status:", error);
      res.status(500).json({ message: "Failed to get status" });
    }
  });

  // POST - Habilitar/desabilitar novo fluxo de vendas
  app.post("/api/admin/sales-flow/:action", isAdmin, async (req, res) => {
    try {
      const action = req.params.action;
      
      if (action === "enable") {
        await storage.updateSystemConfig("sales_flow_enabled", "true");
        res.json({ success: true, message: "Sales flow enabled" });
      } else if (action === "disable") {
        await storage.updateSystemConfig("sales_flow_enabled", "false");
        res.json({ success: true, message: "Sales flow disabled" });
      } else {
        res.status(400).json({ message: "Invalid action. Use 'enable' or 'disable'" });
      }
    } catch (error) {
      console.error("Error controlling sales flow:", error);
      res.status(500).json({ message: "Failed to control sales flow" });
    }
  });
