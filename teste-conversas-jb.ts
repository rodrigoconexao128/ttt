...
const CONVERSAS: ConversaSimulada[] = [
  {
    ...
    validacaoFinal: { deveConter: ['Jennifer', 'R$ 55'], naoDeveConter: ['qual dia', 'qual horário prefere'] }
  },
  {
    id: 2,
    nome: 'Cliente Novo PF - Chuveiro',
    descricao: 'Cliente novo pessoa física quer instalar chuveiro',
    mensagensCliente: ['Boa tarde', 'Não', 'Pessoa física', 'Maria Silva', '12345678901', 'maria@email.com', 'Rua A, 100, Centro, Uberlândia', 'Quero instalar um chuveiro', 'Simples', 'Sim, pode agendar'],
    validacaoFinal: { deveConter: ['Jennifer', 'R$ 95'], naoDeveConter: ['qual dia você prefere'] }
  },
  {
    id: 3,
    nome: 'Cliente Novo PJ - Orçamento',
    descricao: 'Empresa quer orçamento',
    mensagensCliente: ['Olá', 'Não', 'Pessoa jurídica', 'Empresa XYZ Ltda', '12345678000199', 'empresa@xyz.com', 'Av. Brasil, 500, Uberlândia', 'Carlos Gerente', 'Quero um orçamento para a empresa'],
    validacaoFinal: { deveConter: ['Jennifer', 'visita'], naoDeveConter: ['qual dia', 'qual horário'] }
  },
  {
    id: 4,
    nome: 'Bloqueio - Araxá',
    descricao: 'Cliente de Araxá deve ser bloqueado',
    mensagensCliente: ['Oi', 'Sim', 'Preciso de um orçamento', 'Mas é em Araxá'],
    validacaoFinal: { deveConter: ['Uberlândia', 'somente'], naoDeveConter: ['agendar', 'visita', 'horário'] }
  },
  {
    id: 5,
    nome: 'Falta de Energia - CEMIG',
    descricao: 'Cliente sem luz, verificar CEMIG',
    mensagensCliente: ['Oi', 'Sim', 'Estou sem luz em casa', 'Sim, a Cemig disse que é problema interno'],
    validacaoFinal: { deveConter: ['Jennifer', 'agenda'], naoDeveConter: ['qual dia prefere'] }
  },
  {
    id: 6,
    nome: 'Serviço Negado - Cerca',
    descricao: 'Cliente pede cerca elétrica (não fazemos)',
    mensagensCliente: ['Olá', 'Sim', 'Vocês fazem cerca elétrica?'],
    validacaoFinal: { deveConter: ['não'], naoDeveConter: ['vou agendar'] }
  },
  {
    id: 7,
    nome: 'Chuveiro Queimado',
    descricao: 'Cliente com chuveiro queimado',
    mensagensCliente: ['Oi', 'Sim sou cliente', 'Meu chuveiro não está esquentando'],
    validacaoFinal: { deveConter: ['resistência', 'R$ 75', 'Jennifer'], naoDeveConter: ['qual dia'] }
  },
  {
    id: 8,
    nome: 'Ventilador de Teto',
    descricao: 'Cliente quer instalar ventilador',
    mensagensCliente: ['Bom dia', 'Sim', 'Quero instalar um ventilador de teto', 'Sem passagem de fio', 'Pode agendar'],
    validacaoFinal: { deveConter: ['R$ 120', 'Jennifer'], naoDeveConter: ['qual horário você'] }
  },
  {
    id: 9,
    nome: 'Disjuntor Desarmando',
    descricao: 'Cliente com problema no disjuntor',
    mensagensCliente: ['Olá', 'Sim', 'Meu disjuntor fica desarmando'],
    validacaoFinal: { deveConter: ['sobrecarga', 'curto'], naoDeveConter: [] }
  },
  {
    id: 10,
    nome: 'Conversão Voltagem',
    descricao: 'Cliente quer converter voltagem',
    mensagensCliente: ['Boa tarde', 'Sim', 'Quero converter a voltagem de uma tomada', 'Sim, pode agendar'],
    validacaoFinal: { deveConter: ['R$ 55', 'Jennifer'], naoDeveConter: ['qual dia você prefere'] }
  }
];
...
async function executarConversa(conversa: ConversaSimulada): Promise<{sucesso: boolean, historico: any[], problemas: string[]}> {
  ...
  for (const msgCliente of conversa.mensagensCliente) {
    ...
    try {
      ...
      if (resposta.toLowerCase().includes('qual dia') || 
          resposta.toLowerCase().includes('qual horário você prefere') ||
          resposta.toLowerCase().includes('me avisa qual dia')) {
        problemas.push(`Perguntou horário ao cliente: "${resposta.substring(0, 50)}..."`);
      }
  ...
  for (const deve of conversa.validacaoFinal.deveConter) {
    if (!todasRespostas.toLowerCase().includes(deve.toLowerCase())) {
      problemas.push(`Conversa deveria conter "${deve}"`);
    }
  }
  
  for (const naoDeve of conversa.validacaoFinal.naoDeveConter) {
    if (todasRespostas.toLowerCase().includes(naoDeve.toLowerCase())) {
      problemas.push(`Conversa contém "${naoDeve}" mas não deveria`);
    }
  }
  
  const sucesso = problemas.length === 0;
  console.log(`      ${sucesso ? '✅ SUCESSO' : '❌ PROBLEMAS: ' + problemas.length}`);
  ...
  return { sucesso, historico, problemas };
}
