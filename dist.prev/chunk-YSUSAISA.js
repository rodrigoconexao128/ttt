...
async function generateTestToken(userId, agentName, company) {
  ...
  try {
    const { supabase } = await import("./supabaseAuth-XQGH2J5X.js");
...
var ADMIN_CHAT_ATTEMPT_TIMEOUT_MS = 5e3;
...
function buildFastAdminFallback(session, userMessage) {
  ...
  const simpleGreeting = /^(oi|ola|opa|e ai|bom dia|boa tarde|boa noite)$/.test(normalized);
  ...
  if (simpleGreeting) {
    return `${greetingPrefix} Aqui e o Rodrigo, do AgenteZap. Se voce quiser, eu mesmo monto seu teste gratuito aqui e te entrego pronto para conhecer. Me fala o nome do seu negocio que eu comeco.`;
  }
