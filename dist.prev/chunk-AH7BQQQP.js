...
function shouldAutoCreateTestAccount(userMessage, session) {
  ...
  const looksLikeQuestion = userMessage.includes("?") || /^(como|qual|quais|quanto|quando|onde|porque|por que|funciona|serve|da para|d[aá] pra)/.test(normalized);
  return hasStrongIntent && !looksLikeQuestion;
}
...
async function generateTestToken(userId, agentName, company) {
  ...
  try {
    const { supabase } = await import("./supabaseAuth-25ZPKZBF.js");
...
[SISTEMA: O usu\xC3\xA1rio enviou uma m\xC3\xADdia do tipo ${mediaType}. Se for imagem/\xC3\xA1udio sem contexto, pergunte o que \xC3\xA9 (ex: cat\xC3\xA1logo, foto de produto, etc).]`;
  ...
  if (!actionResults.testAccountCredentials && shouldAutoCreateTestAccount(messageText, session)) {
    try {
