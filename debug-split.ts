/**
 * Debug: Simula a divisão de mensagens para encontrar o problema
 */

// Função auxiliar para dividir texto por frases
...
function splitByWords(text: string, maxChars: number): string[] {
  ...
  for (const word of words) {
    ...
    if (combined.length <= maxChars) {
      ...
      // PROBLEMA POTENCIAL: Se palavra > maxChars, CORTA NO MEIO
      if (word.length > maxChars) {
        console.log(`⚠️ ALERTA: Palavra maior que limite (${word.length} > ${maxChars}): "${word.substring(0, 50)}..."`);
        let remaining = word;
...
console.log('🔍 DEBUG: Divisão de Mensagens');
console.log('═'.repeat(80));
...
parts.forEach((part, i) => {
  ...
  if (part.includes('online/') && !part.includes('online/.') && !part.includes('agentezap.online/')) {
    console.log('⚠️ POSSÍVEL URL CORTADA DETECTADA!');
  }
  
  // Verificar se tem "https://" sem o domínio completo
...
console.log('\n' + '═'.repeat(80));
console.log('🔎 Verificando URLs em cada parte:');
console.log('═'.repeat(80));
...
if (fullText.includes(' online/') || fullText.match(/[^.]\s+online\//)) {
