...
function shouldAutoCreateTestAccount(userMessage, session) {
  ...
  const greetingOnly = /^(oi|ola|opa|bom dia|boa tarde|boa noite|e ai)$/.test(normalized);
  return hasStrongIntent || hasMeaningfulMessage && !greetingOnly && userTurns >= 2;
}
...
async function generateTestToken(userId, agentName, company) {
  ...
  try {
    const { supabase } = await import("./supabaseAuth-K57IHJXS.js");
...
function buildFastAdminFallback(session, userMessage) {
  ...
  if (simpleGreeting) {
    return `${greetingPrefix} Aqui e o Rodrigo, do AgenteZap. Eu posso criar seu teste gratuito agora e te mostrar o agente funcionando. Qual o nome do seu negocio?`;
  }
  return `${greetingPrefix} Tive uma lentidao aqui, mas sigo com voce. Me diz o nome do seu negocio e o que voce quer que o agente faca que eu continuo a configuracao.`;
}
