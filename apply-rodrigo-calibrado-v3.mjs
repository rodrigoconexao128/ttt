...
async function main() {
  ...
  console.log(`📄 Novo prompt v3: ${novoPrompt.length} chars`);
  ...
  const backupPath = path.join(__dirname, 'backup-rodrigo-prompt-v3.txt');
  fs.writeFileSync(backupPath, atual[0].prompt, 'utf8');
  console.log(`💾 Backup salvo: backup-rodrigo-prompt-v3.txt (${atual[0].prompt.length} chars)`);

  // Aplica o novo prompt v3
  ...
  if (rowCount === 1) {
    console.log('🎉 Prompt v3 aplicado com sucesso!');
