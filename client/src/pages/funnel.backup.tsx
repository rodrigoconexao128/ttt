...
export default function FunnelPage() {
  ...
  const createDealMutation = useMutation({
    mutationFn: async (data: { funnelId: string; stageId: string; contactName: string; companyName?: string; value?: number; priority?: string; contactPhone?: string; notes?: string }) => {
      const res = await apiRequest("POST", `/api/funnels/${data.funnelId}/stages/${data.stageId}/deals`, {
        contactName: data.contactName, companyName: data.companyName, value: data.value,
        priority: data.priority, contactPhone: data.contactPhone, notes: data.notes,
      });
      return res.json();
    },
    onSuccess: () => {
  ...
  const resetDealForm = () => {
    ...
    setNewDealPriority("Média"); setNewDealPhone(""); setNewDealNotes("");
  ...
  const handleCreateFunnel = () => {
    ...
    createFunnelMutation.mutate({ name: newFunnelName, product: newFunnelProduct || undefined, manager: newFunnelManager || undefined });
  };
  ...
  const handleCreateDeal = () => {
    ...
    createDealMutation.mutate({
      funnelId: selectedFunnelId, stageId: selectedStageForDeal, contactName: newDealName,
      companyName: newDealCompany || undefined, value: newDealValue ? parseFloat(newDealValue) : undefined,
      priority: newDealPriority, contactPhone: newDealPhone || undefined, notes: newDealNotes || undefined,
    });
  };
  ...
  useEffect(() => {
    const interval = setInterval(() => { setLiveUsers(prev => prev + Math.floor(Math.random() * 5) - 2); }, 8000);
  ...
  const computedMetrics = useMemo(() => {
    ...
    let totalDeals = 0, totalValue = 0, stuck = 0;
    ...
    return { totalDeals, stuck, conversion: selectedFunnel.conversionRate || 0, revenue: `R$ ${totalValue.toLocaleString("pt-BR", { minimumFractionDigits: 0 })}` };
  }, [selectedFunnel]);
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <Card className="p-8 text-center max-w-md">
          ...
          <Button onClick={() => queryClient.invalidateQueries({ queryKey: ["/api/funnels"] })}>Tentar novamente</Button>
        </Card>
      </div>
    );
  }
...
export default function FunnelPage() {
  ...
  // Estado para animação de social proof
  const [liveUsers, setLiveUsers] = useState(2847);
  const [recentDeal, setRecentDeal] = useState<string | null>(null);
  
  // Simulação de atividade em tempo real (social proof dinâmico)
  useEffect(() => {
    const interval = setInterval(() => {
  ...
  // Notificação de deal recente
  useEffect(() => {
    const deals = ["TechCorp", "StartupXYZ", "MegaStore", "DigitalBR"];
    const interval = setInterval(() => {
  ...
  return (
    <PremiumBlocked
      ...
      ctaLabel="Ativar Plano Ilimitado"
      benefits={[
        "Pipeline visual drag & drop ilimitado",
        "Automações de WhatsApp por estágio",
        "Métricas de conversão em tempo real",
        "Alertas de deals parados automaticamente",
      ]}
    >
    <div className="flex-1 overflow-auto p-4 md:p-6 bg-gradient-to-br from-slate-50 to-white dark:from-slate-950 dark:to-slate-900">
      <div className="max-w-7xl mx-auto space-y-6">
        
        {/* Notificação de atividade em tempo real - Social Proof */}
        {recentDeal && (
          <div className="fixed top-4 right-4 z-50 animate-in slide-in-from-right duration-500">
            <div className="bg-white rounded-xl shadow-2xl border border-emerald-100 p-4 flex items-center gap-3 max-w-xs">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-400 to-green-500 flex items-center justify-center">
        ...
        {/* Hero Section - Eye-tracking optimizado (informações críticas primeiro) */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-600 via-green-600 to-teal-600 p-6 md:p-8 text-white">
          ...
          <div className="absolute top-4 right-4">
            <Badge className="bg-amber-400/90 text-amber-900 border-0 px-3 py-1 text-xs font-bold animate-pulse">
          ...
          <div className="relative z-10">
            {/* Social Proof Bar */}
            <div className="flex flex-wrap items-center gap-4 mb-6 pb-4 border-b border-white/20">
              <div className="flex items-center gap-2 bg-white/10 rounded-full px-4 py-2 backdrop-blur-sm">
                <div className="flex -space-x-2">
                  {[1,2,3,4].map(i => (
                    <div key={i} className={`w-6 h-6 rounded-full border-2 border-white bg-gradient-to-br 
                ...
                <span className="text-sm font-medium">
                  <strong className="text-white">{liveUsers.toLocaleString()}</strong>
                  <span className="text-emerald-200"> empresas usando</span>
                </span>
              </div>
              <div className="flex items-center gap-2 text-emerald-100 text-sm">
                <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                <span>127 online agora</span>
              </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              ...
              <div className="flex flex-col gap-2">
                <Button 
                  ...
                  className="bg-white text-emerald-700 hover:bg-emerald-50 shadow-xl font-bold text-base px-6 py-6
            ...
            {/* KPIs em destaque - Goal-Gradient Effect */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20
                hover:bg-white/20 transition-all duration-300 hover:scale-105 cursor-pointer group">
                ...
                <div className="mt-2 h-1 bg-white/20 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-emerald-300 to-green-200 rounded-full"
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex-1">
                ...
                <select
                  ...
                  className="w-full sm:w-auto min-w-[240px] border-2 border-slate-200 rounded-lg px-4 py-2.5 
                    text-sm font-medium bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20
              ...
              <div className="flex items-center gap-2 text-sm text-slate-500">
                ...
                <span>{selectedFunnel.produto}</span>
                <span className="text-slate-300">•</span>
                <span>Gestão: {selectedFunnel.responsavel}</span>
              </div>
            </div>
          </CardContent>
        </Card>

        {/* Estágios do Funil - Visual Progress Bar */}
        <Card className="border-0 shadow-sm overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-slate-50 to-white border-b">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <Button variant="outline" onClick={requireSubscription} size="sm" className="gap-2">
          ...
          <CardContent className="p-6">
            ...
            <div className="relative mb-8">
              ...
              <div className="absolute top-5 left-0 h-1 bg-gradient-to-r from-emerald-500 to-green-400 rounded-full" 
                style={{ width: `${(4 / selectedFunnel.stages.length) * 100}%` }} />
                  ...
                  .map((stage, index) => (
                    <div key={stage.id} className="flex flex-col items-center" style={{ width: `${100 / selectedFunnel.stages.length}%` }}>
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold
                        ${index < 4 ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/30' : 
            ...
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
              {selectedFunnel.stages
                ...
                .map((stage) => (
                  <article 
                    key={stage.id} 
                    className={`rounded-xl border-2 ${stage.borderColor} ${stage.bgColor} p-4 space-y-3
                      hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer group`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-2">
                        <div className={`w-8 h-8 rounded-lg ${stage.bgColor} border ${stage.borderColor} 
                          flex items-center justify-center font-bold text-sm ${stage.color}`}>
                          {stage.position}
                        </div>
                        <div>
                          <p className={`font-semibold ${stage.color}`}>{stage.nome}</p>
                          <p className="text-[10px] text-slate-500">{stage.descricao}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2 pt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button size="sm" variant="outline" onClick={requireSubscription} className="flex-1 h-8 text-xs">
        ...
        {/* Pipeline Visual - Kanban Style Premium */}
        <Card className="border-0 shadow-sm overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-slate-50 to-white border-b">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
              ...
              <div className="flex gap-2">
                <Button variant="outline" onClick={requireSubscription} size="sm">
                  Exportar
                </Button>
                <Button onClick={requireSubscription} size="sm" className="bg-emerald-600 hover:bg-emerald-700 gap-2">
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4 min-h-[420px]">
              {selectedFunnel.stages
                ...
                .map((stage) => (
                  <section
                    key={stage.id}
                    className={`flex flex-col rounded-xl border-2 ${stage.borderColor} bg-white
                  ...
                  >
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <div className={`w-3 h-3 rounded-full bg-gradient-to-r 
                            ${stage.position === 1 ? 'from-slate-400 to-slate-500' : 
                              stage.position === 2 ? 'from-blue-400 to-blue-500' :
                              stage.position === 3 ? 'from-amber-400 to-amber-500' :
                              'from-purple-400 to-purple-500'}`} />
                          <p className={`font-bold ${stage.color}`}>{stage.nome}</p>
                        </div>
                    ...
                    <div className="flex-1 space-y-3 p-3 min-h-[280px]">
                      {stageDeals(stage.id).map((deal) => (
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex items-center gap-2">
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs
                                ${deal.prioridade === 'Alta' ? 'bg-gradient-to-br from-red-400 to-rose-500' :
                                  deal.prioridade === 'Média' ? 'bg-gradient-to-br from-amber-400 to-orange-500' :
                                  'bg-gradient-to-br from-blue-400 to-blue-500'}`}>
                                {deal.contato.charAt(0)}
                              </div>
                              <div>
                                <p className="font-semibold text-sm text-slate-800">{deal.contato}</p>
                                <p className="text-[10px] text-slate-500">{deal.empresa}</p>
                              </div>
                            </div>
                            <Badge className={`text-[10px] font-semibold border
                              ${deal.prioridade === 'Alta' ? 'bg-red-50 text-red-700 border-red-200' :
                                deal.prioridade === 'Média' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                          ...
                          <div className="mt-3 p-2 bg-emerald-50 rounded-lg">
                            <p className="text-lg font-bold text-emerald-700">{deal.valor}</p>
                          </div>
                          ...
                          <div className="mt-3 space-y-1.5 text-xs text-slate-500">
                            ...
                            <div className="flex items-center gap-1.5">
                              <Clock className={`w-3 h-3 ${
                                deal.ultimoContato.includes('dias') ? 'text-amber-500' : 'text-emerald-500'
                              }`} />
                              <span className={deal.ultimoContato.includes('dias') ? 'text-amber-600 font-medium' : ''}>
                                {deal.ultimoContato}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="mt-3 pt-3 border-t flex gap-2">
                            ...
                            <Button size="sm" onClick={requireSubscription} className="flex-1 h-8 text-xs bg-emerald-600 hover:bg-emerald-700">
                      ...
                      {stageDeals(stage.id).length === 0 && (
                        <div className="h-full min-h-[200px] rounded-xl border-2 border-dashed border-slate-200
                          flex flex-col items-center justify-center text-center p-4 bg-slate-50/50">
        ...
        {/* CTA de Conversão Final - Bottom of Page */}
              ...
              <div className="flex flex-col items-center gap-4">
                <Button 
                  ...
                  className="bg-gradient-to-r from-emerald-500 to-green-500 hover:from-emerald-600 hover:to-green-600
                    text-white font-bold text-lg px-8 py-6 shadow-xl shadow-emerald-500/30
                    transform hover:scale-105 transition-all duration-300"
                >
