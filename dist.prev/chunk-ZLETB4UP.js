...
function hasGeneralEditIntent(message) {
  const normalized = normalizeTextToken(message);
  if (!normalized) return false;
  return /\b(editar|edita|alterar|altera|mudar|muda|ajustar|ajusta|calibrar|calibra|corrigir|corrige|mexer|revisar|revisa)\b/.test(normalized);
}
...
function hasStartedGuidedSetup(session) {
  ...
  return Boolean(
    profile?.answeredBusiness || profile?.answeredBehavior || profile?.answeredWorkflow || profile?.questionStage === "hours" || profile?.questionStage === "ready"
  );
}
...
function looksLikeCurrentGuidedAnswer(profile, message) {
  ...
  if (!profile.answeredBusiness) {
    return Boolean(
      extractBusinessNameCandidate(message) || sanitizeCompanyName(message) || extractMainOfferFromBusinessSummary(message)
    );
  }
  if (!profile.answeredBehavior) {
    return normalized.includes("quero que ele") || normalized.includes("quero que o agente") || /\b(venda|vender|follow[ -]?up|duvida|duvidas|agenda|agendamento|agendar|cobran|cobrar|recuperar|suporte|comercial|qualifica|responder|fechar|atender|mistur)\b/.test(normalized);
  }
  if (!profile.answeredWorkflow) {
    const parsedHours = parseWorkWindow(message);
    return Boolean(
      parseRestaurantOrderMode(message) || parseSchedulingPreference(message, { allowPlainYesNo: false }) !== void 0 || parseGenericWorkflowFollowUpPreference(message) !== void 0 || parseWorkDays(message)?.length || parsedHours.workStartTime || parsedHours.workEndTime
    );
  }
...
function isOnboardingSideQuestion(message, profile) {
  ...
  return /\b(plano|preco|valor|mensalidade|assinatura|quanto custa|pix|pagamento)\b/.test(normalized) || /\b(como funciona|funciona|como conecta|conectar|whatsapp|teste|suporte)\b/.test(normalized) || /\b(audio|video|foto|imagem|midia|midea|crm|kanban|follow[ -]?up|notificador)\b/.test(normalized);
}
...
function parseSchedulingPreference(message, options) {
  const normalized = normalizeTextToken(message);
  if (!normalized) return void 0;
  if (normalized.includes("nao agenda") || normalized.includes("n\xE3o agenda") || normalized.includes("sem agendamento") || normalized.includes("nao precisa agendar") || normalized.includes("n\xE3o precisa agendar") || normalized.includes("so responde") || normalized.includes("s\xF3 responde") || normalized.includes("somente venda") || normalized.includes("somente vendas") || normalized.includes("so venda") || normalized.includes("so vendas") || normalized.includes("s\xF3 venda") || normalized.includes("s\xF3 vendas") || normalized.includes("apenas venda") || normalized.includes("apenas vendas") || normalized.includes("somente comercial") || normalized.includes("so comercial") || normalized.includes("s\xF3 comercial")) {
    return false;
  }
...
function hasSchedulingSignal(message) {
  const normalized = normalizeTextToken(message);
  if (!normalized) return false;
  return normalized.includes("agendamento") || normalized.includes("agendar") || normalized.includes("agenda") || normalized.includes("horario") || normalized.includes("hor\xE1rio") || normalized.includes("consulta") || normalized.includes("reservar") || normalized.includes("reserva");
}
...
function parseGenericWorkflowFollowUpPreference(message) {
  const normalized = normalizeTextToken(message);
  if (!normalized) return void 0;
  if (normalized.includes("follow up") || normalized.includes("follow-up") || normalized.includes("recuperar cliente") || normalized.includes("recuperar quem nao respondeu") || normalized.includes("recuperar quem n\xE3o respondeu") || normalized.includes("continuar tentando") || normalized.includes("voltar a falar") || normalized.includes("correr atras") || normalized.includes("correr atr\xE1s")) {
    return true;
  }
  if (normalized.includes("somente venda") || normalized.includes("somente vendas") || normalized.includes("so venda") || normalized.includes("so vendas") || normalized.includes("s\xF3 venda") || normalized.includes("s\xF3 vendas") || normalized.includes("apenas venda") || normalized.includes("apenas vendas") || normalized.includes("s\xF3 atender") || normalized.includes("so atender") || normalized.includes("me avisa") || normalized.includes("me chamar") || normalized.includes("me chama") || normalized.includes("te avisa") || normalized.includes("te chama") || normalized.includes("somente comercial") || normalized.includes("so comercial") || normalized.includes("s\xF3 comercial")) {
    return false;
  }
  if (/\bsim\b/.test(normalized)) return true;
  if (/\bnao\b/.test(normalized)) return false;
...
function buildStructuredAgentInstructions(session) {
  ...
  if (workflowKind === "generic" && typeof profile?.wantsAutoFollowUp === "boolean") {
    parts.push(
      profile.wantsAutoFollowUp ? "Depois do primeiro atendimento, fa\xE7a follow-up autom\xE1tico de forma natural para recuperar quem sumiu e continuar a venda." : "N\xE3o force follow-up autom\xE1tico em todo caso. Foque em atendimento e vendas, e s\xF3 chame o respons\xE1vel quando realmente precisar."
    );
  }
  parts.push(
...
function getGuidedWorkflowQuestion(profile) {
  ...
  return "Fechado. 3) Depois do primeiro contato, voc\xEA quer que ele fa\xE7a follow-up autom\xE1tico para recuperar quem n\xE3o respondeu ou prefere que ele s\xF3 atenda, venda e te avise quando precisar de voc\xEA?";
}
...
function parseExistingClientPromptAdjustments(message) {
  ...
  const moreCommercial = normalized.includes("mais comercial") || normalized.includes("tom comercial") || normalized.includes("mais vendedor") || normalized.includes("tom de vendedor");
  let agentName;
  let company;
  const identityMatch = String(message || "").match(
    /(?:identific(?:a|ar|ando)(?:-?se)?|apresent(?:a|ar)(?:-?se)?|come[cç]a(?:r)?(?:\s+se)?\s+identificando)\s+como\s+([^.!?\n]+)/i
  );
  const identityRaw = identityMatch?.[1]?.replace(/^["'`]+|["'`]+$/g, "").replace(/\s+/g, " ").trim();
  if (identityRaw) {
  ...
  return {
    requested: Boolean(identityRaw || moreCommercial),
    agentName,
    company,
    moreCommercial
  };
...
function applyExistingClientPromptAdjustments(currentPrompt, updates) {
  ...
  let changed = false;
  if (company) {
    const identityLine = `Voc\xEA \xE9 ${agentName}, ${role} da ${company}.`;
    ...
    const personaLine = `9. PERSONA: Se perguntarem quem \xE9, diga 'Sou ${agentName} da ${company}'. Nunca diga 'Sou um assistente virtual'.`;
    if (/9\.\s*PERSONA:[^\n]*/i.test(nextPrompt)) {
...
${personaLine}`;
    ...
    const greetingExample = `${agentName}: "Ol\xE1! \u{1F44B} Bem-vindo \xE0 ${company}! Como posso te ajudar hoje?"`;
    if (/Cliente:\s*"Oi"\s*\n[^\n]+:\s*"[^"]*"/i.test(nextPrompt)) {
  ...
  if (updates.moreCommercial) {
    const commercialLine = "Use um tom mais comercial, mas natural, focado em convers\xE3o e em conduzir a venda sem parecer rob\xF4.";
...
async function maybeApplyStructuredExistingClientUpdate(session, userMessage) {
  ...
  const promptAdjustments = parseExistingClientPromptAdjustments(userMessage);
  ...
  if (!hasScheduleUpdate && !restaurantOrderMode && genericFollowUpPreference === void 0 && !promptAdjustments.requested) {
  ...
  if (promptAdjustments.requested) {
    const currentConfig2 = await storage.getAgentConfig(session.userId);
    const currentPrompt2 = currentConfig2?.prompt || "";
    const adjustedPrompt = applyExistingClientPromptAdjustments(currentPrompt2, {
    ...
    if (adjustedPrompt.changed) {
      ...
      if (adjustedPrompt.company || adjustedPrompt.agentName) {
        ...
        const nextConfig = {
          ...
          role: adjustedPrompt.company ? inferRoleFromBusinessName(adjustedPrompt.company) : session.agentConfig?.role,
          prompt: adjustedPrompt.prompt
        };
        session = updateClientSession(session.phoneNumber, { agentConfig: nextConfig });
      }
      await consumeAdminPromptEdit(session.userId);
      const identityLine = adjustedPrompt.company ? `${adjustedPrompt.agentName || "o atendente"} da ${adjustedPrompt.company}` : adjustedPrompt.agentName || "do jeito que voc\xEA pediu";
      const toneLine = promptAdjustments.moreCommercial ? " Tamb\xE9m deixei o tom mais comercial." : "";
      return {
        applied: true,
        text: `Fechado. J\xE1 atualizei o prompt real do seu agente para ele se apresentar como ${identityLine}.${toneLine} Testa no mesmo link do simulador agora e, se quiser, eu sigo ajustando.`
      };
    }
  }
  ...
  } else if (genericFollowUpPreference !== void 0 && currentWorkflow === "generic") {
...
Hor\xE1rio operacional real: ${formatBusinessDaysForHumans(profile.workDays)}, das ${profile.workStartTime || DEFAULT_WORK_START} \xE0s ${profile.workEndTime || DEFAULT_WORK_END}. Nunca ofere\xE7a hor\xE1rios fora dessa janela.` : "";
  const genericFlowInstruction = workflowKind === "generic" && typeof profile.wantsAutoFollowUp === "boolean" ? profile.wantsAutoFollowUp ? "Depois do primeiro atendimento, fa\xE7a follow-up autom\xE1tico de forma natural para recuperar quem sumiu e continuar a venda." : "N\xE3o force follow-up autom\xE1tico em todo caso. Foque em atendimento e vendas, e s\xF3 chame o respons\xE1vel quando realmente precisar." : "";
  if (currentPrompt) {
    let nextPrompt = currentPrompt;
    if (scheduleBlock) {
      if (nextPrompt.includes("Hor\xE1rio operacional real:")) {
    ...
    if (genericFlowInstruction) {
      const promptWithoutOldGenericFlow = nextPrompt.replace(
        /Depois do primeiro atendimento, faça follow-up automático de forma natural para recuperar quem sumiu e continuar a venda\./i,
        ""
      ).replace(
        /Não force follow-up automático em todo caso\. Foque em atendimento e vendas, e só chame o responsável quando realmente precisar\./i,
        ""
      ).replace(/\n{3,}/g, "\n\n").trim();
...
${genericFlowInstruction}`.trim();
  ...
  return {
    applied: true,
    text: workflowKind === "delivery" && restaurantOrderMode ? `Fechei isso pra voc\xEA. O modo do restaurante j\xE1 foi ajustado para ${restaurantOrderMode === "full_order" ? "pedido completo" : "primeiro atendimento"} e o m\xF3dulo correspondente j\xE1 ficou alinhado em https://agentezap.online${panelPath}. Se quiser, testa no mesmo link do simulador agora.` : workflowKind === "generic" && genericFollowUpPreference !== void 0 ? `Fechado. J\xE1 alinhei o agente para ${genericFollowUpPreference ? "continuar com follow-up autom\xE1tico depois do primeiro contato" : "focar em atendimento e vendas sem insistir em follow-up"} e atualizei o prompt dessa conta. Se quiser, testa no mesmo link do simulador agora.` : `Fechou. J\xE1 atualizei os dias e hor\xE1rios reais no m\xF3dulo ${workflowKind === "salon" ? "de sal\xE3o" : "de agendamentos"} e tamb\xE9m alinhei o agente. Agora ele s\xF3 deve trabalhar dentro de ${formatBusinessDaysForHumans(profile.workDays)}, das ${profile.workStartTime || DEFAULT_WORK_START} \xE0s ${profile.workEndTime || DEFAULT_WORK_END}. Se quiser, testa no mesmo link do simulador agora.`
  };
}
...
function sessionHasDeliveredTestLink(session) {
  ...
  return session.conversationHistory.some((item) => {
    if (item.role !== "assistant" || !item.content) return false;
    const content = String(item.content);
    return content.includes("/test/") || content.includes("agentezap.online/login");
  });
}
...
async function maybeHandleGuidedOnboardingTurn(session, userMessage, options) {
  ...
  if (!profile.answeredWorkflow) {
    ...
    if (profile.workflowKind === "delivery") {
      ...
      const schedulingPreference = parseSchedulingPreference(cleanMessage, { allowPlainYesNo: useSchedulingQuestion || profile.workflowKind === "salon" }) ?? (profile.workflowKind === "salon" ? true : void 0);
      if (useSchedulingQuestion) {
        ...
        if (schedulingPreference === true) {
          ...
          profile.answeredWorkflow = true;
          profile.questionStage = shouldRequireHours(profile) ? "hours" : "ready";
        } else if (genericFollowUpPreference !== void 0 || schedulingPreference === false) {
          profile.usesScheduling = false;
...
[SISTEMA: O usu\xC3\xA1rio enviou uma m\xC3\xADdia do tipo ${mediaType}. Se for imagem/\xC3\xA1udio sem contexto, pergunte o que \xC3\xA9 (ex: cat\xC3\xA1logo, foto de produto, etc).]`;
  ...
  const { getSmartTriggers } = await import("./adminMediaStore-2H5Q6BEJ.js");
...
async function maybeHandleDirectConversationTurn(session, userMessage, linkedContext, options) {
  ...
  if (wantsEdit && !hasLinkedUser && !onboardingInProgress) {
    return {
      handled: true,
