...
final class MaskPattern
{
     ...
     */
    public const AUTO = -1;
    public const PATTERN_000 = 0b0;
    public const PATTERN_001 = 0b1;
    public const PATTERN_010 = 0b10;
    public const PATTERN_011 = 0b11;
     ...
     * @var int[]
     */
    public const PATTERNS = [self::PATTERN_000, self::PATTERN_001, self::PATTERN_010, self::PATTERN_011, self::PATTERN_100, self::PATTERN_101, self::PATTERN_110, self::PATTERN_111];
    /*
     * Penalty scores
     *
    ...
    public function getMask() : Closure
    {
        ...
        return [self::PATTERN_000 => fn(int $x, int $y): bool => ($x + $y) % 2 === 0, self::PATTERN_001 => fn(int $x, int $y): bool => $y % 2 === 0, self::PATTERN_010 => fn(int $x, int $y): bool => $x % 3 === 0, self::PATTERN_011 => fn(int $x, int $y): bool => ($x + $y) % 3 === 0, self::PATTERN_100 => fn(int $x, int $y): bool => (intdiv($y, 2) + intdiv($x, 3)) % 2 === 0, self::PATTERN_101 => fn(int $x, int $y): bool => $x * $y % 6 === 0, self::PATTERN_110 => fn(int $x, int $y): bool => $x * $y % 6 < 3, self::PATTERN_111 => fn(int $x, int $y): bool => ($x + $y + $x * $y % 3) % 2 === 0][$this->maskPattern];
    }
    /**
    ...
    public static function getBestPattern(QRMatrix $QRMatrix) : self
    {
        ...
        foreach (self::PATTERNS as $pattern) {
            $mp = new self($pattern);
            $matrix = (clone $QRMatrix)->setFormatInfo($mp)->mask($mp)->getMatrix(\true);
            $penalty = 0;
        ...
        return new self(array_search(min($penalties), $penalties, \true));
    }
    /**
    ...
    public static function testRule2(array $matrix, int $height, int $width) : int
    {
        ...
        foreach ($matrix as $y => $row) {
            ...
            foreach ($row as $x => $val) {
                ...
                if ($val === $row[$x + 1] && $val === $matrix[$y + 1][$x] && $val === $matrix[$y + 1][$x + 1]) {
                    $penalty++;
                }
            }
        }
    ...
    public static function testRule3(array $matrix, int $height, int $width) : int
    {
        ...
        foreach ($matrix as $y => $row) {
            foreach ($row as $x => $val) {
                if ($x + 6 < $width && $val && !$row[$x + 1] && $row[$x + 2] && $row[$x + 3] && $row[$x + 4] && !$row[$x + 5] && $row[$x + 6] && (self::isWhiteHorizontal($row, $width, $x - 4, $x) || self::isWhiteHorizontal($row, $width, $x + 7, $x + 11))) {
                    $penalties++;
                }
                if ($y + 6 < $height && $val && !$matrix[$y + 1][$x] && $matrix[$y + 2][$x] && $matrix[$y + 3][$x] && $matrix[$y + 4][$x] && !$matrix[$y + 5][$x] && $matrix[$y + 6][$x] && (self::isWhiteVertical($matrix, $height, $x, $y - 4, $y) || self::isWhiteVertical($matrix, $height, $x, $y + 7, $y + 11))) {
                    $penalties++;
                }
            }
        }
