  ...
  async createClientCheckout(params: CheckoutParams): Promise<CreateClientResult> {
    ...
    try {
      ...
      if (paymentMethod === 'pix') {
        // Gerar PIX
        const pixResult = await generatePixQRCode({
          planNome: `Cliente ${clientData.name}`,
          valor: costPerClient,
          subscriptionId: externalReference,
        });

        // Criar registro temporário de pagamento pendente
        ...
        // Salvar dados do cliente no payment para criar depois do pagamento
